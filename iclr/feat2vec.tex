...
Surprisingly, very little work has focused on methods for structured datasets where there is more than one feature type---that is, datasets that have arbitrary features beyond words.
...
For example, consider multiple discrete entity types, like universities, students, and textbooks.
...
\hl{This choice places more weight on features that have more parameters and thus are going to require more training iterations to properly learn.}
For categorical variables using a linear fully-connected layer, the complexity is simply \hl{proportional to }the number of categories in the feature type:
\begin{equation}
P_{\q_1}(\upkappa_i|\operatorname{params}(\phi_i)\}_{i=1}^n)  \
= \frac{(r\times d_i)^{\alpha_1}}{\sum_{k=1}^n \operatorname{params}(\phi_k)^{\alpha_1}},  \quad \alpha_1 \in [0,1]
\end{equation}
...
With $n$ feature types, $\Feat2Vec$ is equivalent to optimizing a convex combination of the embeddings learned from $n$ individual Factorization Machines.
In other words, it optimizes $n$ multi-label classifiers, where each classifier  is optimized for a different target (i.e., feature type).
...
Thus, our evaluation approach we use the embeddings of one entity, and evaluate its nearest-neighbors from a different entity from which we know the ground-truth.
...
\hl{In each setting, $\alpha_2$ is set to the same flattening hyperparameter we use for $\Word2Vec$ to negatively sample words in a document.} We learn $r=50$ dimensional embeddings under both algorithms.
...
\begin{figure}
\centering
\includegraphics[scale=.6]{../paper/output/ratingsRankingRMSE.pdf}
...
While finishing this paper, we recently discovered a promising direction for an algorithm still in development called StarSpace~\cite{wu2017starspace} with similar goals from ours.
However, eventhough they intend to be able to embed ``all things'', at the time of writing this paper, their pre-print method was constrained to only work for bag of words.
While $\Feat2Vec$ is a convex combination of all possible prediction tasks given the featureset,  StarSpace samples a single arbitrary feature.
...
These ideas can extend to our codebase that we make available\footnote{\hl{dropbox link here?}}.
...
To the extent of our knowledge, $\Feat2Vec$  is the first method able to calculate continuous representations of data with arbitrary feature sets.
