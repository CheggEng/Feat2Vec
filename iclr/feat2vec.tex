...
\paragraph{Features extraction functions} 
\hl{Jose is working on this section, but has questions}
\begin{itemize}
\item For all of the categorical variables, we learn a unique $r$-dimensional embedding for each entity.
When there are multiple categories per record, we sum the individual entity embeddings.
...
\item For the single text feature we use (the Movie Title), we remove preprocess the text by removing non alpha-numeric characters, pruning stopwords, and stemming the remaining words in the title text.
...
\item For all real-valued features, we pass these features through a 3-layer feed-forward fully connected neural network that outputs a vector of dimension $r$, which we treat as the feature's embedding.
...
In contrast, $\Word2Vec$ would be unable to know ex-ante that an IMDB rating of 5.5 is anything like an IMDB rating of 5.6; it would tokenize both values and treat them as independent words in a document.
