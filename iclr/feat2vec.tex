...
\hl{Luis: please write a few sentences justifying why we do this? i.e., what's the advantage? Right now we don't have experiments supporting this.}
...
\hl{how do you do c.v. with no dev set? do you tune only f2v, or also w2v?}
...
%Cite keras???
 For training $\Feat2Vec$  we set $\alpha_1 = 1/4$ and $\alpha_2 = 3/4$ in the IMDB dataset;
 and $\alpha_1 = 0$ and $\alpha_2 = 0.5$  for the educational \hl{confirm?}.
...
In fact,  $\Feat2Vec$ predicts the actual director 2.43\% of the times, while $\Word2Vec$ only does so 1.26\% of the time, being our approach almost twice better in terms of Top-1 Precision metric.
...
$\Feat2Vec$ needs domain knowledge about how to group features into entities;
and  requires  an expert to choose or  design a feature extraction function.
We  intend to facilitate progress on this by making our code and  most of our experiments  available\footnote{\hl{dropbox link here?}}.
...
To the extent of our knowledge, we propose the first algorithm that is able to calculate continuous representations of data with arbitrary feature sets.
