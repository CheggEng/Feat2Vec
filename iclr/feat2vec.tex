...
In \S~\ref{sec:learning} we propose a supervised model to learn a $\Feat2Vec$ model from training data.
This method requires positive and negative examples of features co-ocurrence.
However, our data consists of only implicit feedback.
Thus, in \S~\ref{sec:sampling}, we describe how to  generate implicit samples.
...
To operationalize this, Structured Deep-In Factorization Machine defines $\vect{\upkappa}$  as a vector of vectors that define  groups of features. 
...
Thus:
 \[
 \vect{x}_{\vect{\upkappa}_i} =  \bigl[ \, x_{i_a} | a \in \{1, 2, \dots, {|\upkappa_i|} \} \, \bigr]
 \]
