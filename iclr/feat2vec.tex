...
\hl{The idea behind this choice is to weigh more heavily features that have more parameters/features and thus are going to require more training iterations to properly learn. If we were to naively sample each feature with equal probability in the first step of Algorithm 1, the algorithm would spend too little time resampling features with sparse elements.}
...
\end{equation}
However, if we have multiple intermediate layers for complex feature types (e.g., convolutional layers), these parameters should also be counted.
...
\hl{In each setting, $\alpha_2$ is set to the same flattening hyperparameter we use for $\Word2Vec$ to negatively sample words in a document.}
...
In fact,  $\Feat2Vec$ predicts the actual director 2.43\% of the times, while $\Word2Vec$ only does so 1.26\% of the time, making our approach almost twice better in terms of Top-1 Precision metric.
