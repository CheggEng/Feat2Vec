...
\newcommand{\citealp}[1]{\citeauthor{#1} ƒ\citeyear{#1}}
...
$\Feat2Vec$ predicts  a target output  $\tilde y$ from observations that can be interpreted as a list of feature groups:
\begin{align}
...
Thus, each observation is constructed from a partition of raw features ${\vect{\mathbf x}}$
(e.g,  a group can be an image, and a raw feature can be a pixel).
...
\tilde y(\vect{{x}\vphantom{phi}},\vec{\pmb{\upphi}}, \vect{\pmb{\uptheta}\vphantom{phi}} ) = \omega \biggl(
                       \sum_{i =1}^n \sum_{j=i}^n  \overbracket {\phi_i( \vect{x \vphantom{\uptheta} }_{\vect{\upkappa}_i};  \vect{\pmb{\uptheta}}) }^{ \mathclap  { \text{group $i$}\atop \text{embedding}}} \dotp \overbracket {\phi_j( \vect{x \vphantom{\uptheta} }_{\vect{\upkappa}_j}; \vect{\pmb{\uptheta}  }) }^{ \mathclap  { \text{group $j$}\atop \text{embedding}}}
  \biggr)
 \label{eq:deepfm}
\end{align}

Here, $\omega$ is an activation (link) function.
...
We can learn the the parameters of a Feat2Vec model $ \bm\uptheta$  by minimizing a supervised loss function $\mathcal L_\text{sup}$:
\label{sec:learning}
\begin{equation}
\arg \min_{\bm{\uptheta}} \sum_{\vect{x} \in \mathbf{X}}  \mathcal{L}_\text{sup}\bigl( \, y(\vect{x}), \tilde y(\vect{x};{\bm \upphi, \bm\uptheta}) \, \bigr)
%+ \gamma||  \bm{\uptheta}||^{w}
\label{eq:learning}
\end{equation}
Here, $\mathbf X$ is the training dataset, $y(\vect{x})$  is the true target value for an observation $\vect{x} \in \mathbf{X}$, and $\tilde {y}(\vect{x})$ is its estimated value (using Equation \ref{eq:deepfm});
$\bm\uptheta$ represents the parameters learned  during training (i.e. the parameters associated with the extraction functions $\phi_i$).
...
Instead, by leveraging on the feature groups $\vect{\pmb{\upkappa}}$ of the data (Equation \ref{eq:deepfm}), the model can reason on more abstract entities in the data.
...
We start with a training dataset $\mathbf X^+ $ of records with $n$ feature groups that we assume to have a positive label.
...
Consider that we have a dataset with three feature groups---entities for (i) a passage of text (bag of words), (ii) a restaurant identifier (categorical), and a sentiment (a continuous value).
The positive examples are observations of the values seen in the training set; while negative examples are generated by modifying observations by substituting one of the entities randomly from another observation.
For example, by substituting the passage of text, a negative example may result as:
...
More formally, our negative sampling generates observations with a negative label from an observation in the training dataset (that is assumed to have a positive label).
...
= \frac{ \operatorname{count}(\vect{g} )^{\alpha_2}}{\sum\limits_{\vect{j}' \in \mathbf{X}^+_{\upkappa_i} } \operatorname{count}(\vect{j})^{\alpha_2}}, \quad \alpha_2 \in [0,1]
\end{equation}
Here, $\operatorname{count}(\vect{g})$ is the number of times a value $\vect{g}$ of feature group $i$ appeared in the training dataset $\mathbf{X}^+$ .
...
who show that in models with many parameters setting the $Z_{\vec{x}}=1$ does not change the performance of the model.
...
We adjust the structural statistical model of Equation~\ref{eq:deepfm} to handle self-supervised learning.
Because we only deal with a dichotomous label (positive or negative example), we restrict our attention to the logistic link function.
Written explicitly, this is:
\begin{align}
\tilde p(y=1|\vec{x\vphantom{phi}},\vect{\bm\upphi},\vect{\bm\uptheta}) &=
 \frac{\exp\bigl(\tilde y (\vec{x\vphantom{phi}}|\vect{\bm\upphi},\vect{\bm\uptheta}) \bigr)}{\exp( \tilde y \bigl(\vec{x\vphantom{phi}}, \vect{\bm\upphi},\vect{\bm\uptheta}) \bigr) + P_{\q}(\vec{\vphantom{phi}x}| \alpha_1,\alpha_2)} \nonumber \\
 & \triangleq \hat s(\vect{x\vphantom{\uptheta}};\vect{\bm\uptheta})
\end{align}
For notational convenience, we refer to this structural probability as $\hat s(\cdot; \vect{\bm\uptheta})$.
 $P_{\q}(x)$ denotes the probability of $x$ under distribution $\q$;
 it accounts for the possibility of sampling an observation with a negative label that appears identical to one in the training data (with a positive label):
\begin{align}
P_{\q}(\vec{x}|\alpha_1,\alpha_2,\mathrm{X},\vect{x}^+) = P_{\q_2}(\vec{x}_{\upkappa_i}|\mathrm{X}_{\upkappa_i},\alpha_2)\times \nonumber \\ 
                                                                                            P_{\q_1}(\upkappa_i|\operatorname{params}(\phi_i)\}_{i=1}^n,\alpha_1)
\end{align}
%The details of the sampling of observations with negative labels is explained in \S~\ref{sec:sampling}.

We seek to optimize the parameters $\bm\uptheta$ of the feature extraction functions $\vect\phi$:
\begin{equation}
\arg\min_{\bm\uptheta} \sum_{\vect{x}^{\,+} \in X^+}  \mathcal{L}_\text{self} \bigl( {\vect{x}^{\,+}}; \bm\uptheta  \bigr)
\end{equation}
For this, we define our self-supervised loss as $\mathcal{L}_\text{self}$ as: 
\begin{align}
%-  \sum_{\vect{x}^{\,+} \in S^+} \Big(\log(\hat p(y=1|\vect{x}^{\,+},\bm\uptheta   ) ) +  \textstyle\sum\limits_{\vect{x}^{\,-}  \sim \q(\cdot|\vect{x}^+)}^k \log(\hat p(y=0|\vect{x}^{\,-},\bm\uptheta   )) \Big)
-\sum_{\vect{x}^{\,+} \in \mathbf{X}^+} \biggl( \log\bigl(\hat s(\vect{x}^{\,+};{\bm\uptheta} )   \bigr) +  \textstyle\sum\limits_{\vect{x}^{\,-}  \sim \q(\cdot|\vect{x}^+)}^k \log\bigl(1-\hat s(\vect{x}^{\,-};{\bm\uptheta}) \bigr) \biggr)
\end{align}
