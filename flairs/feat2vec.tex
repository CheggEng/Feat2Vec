...
We study how to estimate  dense representations when multiple feature types exist within a dataset for  supervised learning where explicit labels are available, as well as for self-supervised learning where there are no labels.
...
In the self-supervised case, our experiments suggest that $\Feat2Vec$ significantly outperforms existing algorithms that do not leverage the structure of the data.
...
So an embedding of a movie learned to predict ratings, it is unlikely to be extremely helpful for an unrelated prediction task.
...
In this paper we propose $\Feat2Vec$ as a novel method that allows calculating embeddings of arbitrary feature types for both supervised and self-supervised scenarios.
...
the authors of the seminal Doc2Vec~\cite{le2014distributed} paper needed to design a new neural network and a new sampling strategy  to simply add a single feature.
...
% $\Feat2Vec$ extends Factorization Machine in two ways:
% it allows grouping of features and it enables arbitrary feature extraction functions.
We now describe how $\Feat2Vec$ extends the Factorization Machine model by allowing grouping of features, and enabling arbitrary feature extraction functions (\S~\ref{sec:algoralph}).
We also report a supervised method to learning Feat2Vec (\S~\ref{sec:learning_supervised}),
as well as a novel self-supervised training procedure (\S~\ref{sec:sampling}).

\subsection{Model}
\label{sec:algoralph}

We propose a framework for extending factorization machine with neural methods, by introducing structure into the feature interactions.
Specifically, we do this by defining  \textit{feature groups},  $\vect{\upkappa}$, where each group contains features of a particular type. Explicitly, $\vect{\upkappa}$ is a partition of the set of feature columns in a dataset and each set within the partition is a feature group. The embeddings of a feature group are then learned via a \textit{feature extraction function}, $\phi_i$, defined for each feature group.
...
$\phi_i$ is a feature extraction that inputs the $i$-th feature group of the instance, and returns an $r$-dimensional embedding. The feature extraction function $\phi_i$ can allow for an arbitrary processing of its subfeatures.
Across groups,  entities interact with  each other via the output of $\phi$ only.
...
Note that without loss of generality, we could define a model that is equivalent to a shallow Factorization Machine by allowing each feature group to be a singleton :
$
...
For the labeling and classification tasks, we optimize the binary cross-entropy
...
In the multi-class scenario, if the number of labels is very large, it is common practice use a binary classifier   and use implicit sampling~\cite{samplingnotes}.
...
In short, $\Word2Vec$  samples a negative observation from a noise distribution $\q_{w2v}$, that is proportional to the empirical frequency of a word in the training data.

We introduce a new implicit sampling method that enables learning self-supervised embeddings for structured feature sets.
...
\begin{algorithm}[htb]
 \caption{Implicit sampling algorithm for self-supervised $\Feat2Vec$: $\q$}
\begin{algorithmic}[1]
\footnotesize
...
To sample a value from within a feature groups $\upkappa_i$, we use a similar strategy to $\Word2Vec$ and use the empirical distribution of values:
...
Alternatively, it is possible to account for the probability of random negative labels that are identical to positively labeled data using Noise Contrastive Estimation (NCE)~\cite{nce}.
...
expressed in Equation \ref{eq:deepfm} to account for the possibility of random negative labels that appear identical to positively labeled data. $\bm\uptheta$ here represents the parameters learned in during training (i.e. the $b_i$ terms and parameters associated with the extraction functions $\phi_i$ in Equation \ref{eq:deepfm}). Since we only deal with a dichotomous label, indicating a positive or negative sample, for self-superrvised learning, we restrict our attention to usage of Equation \ref{eq:deepfm} with $\omega$ as a logistic link function.
...
with at least two features, one can use one of the feature as the target label, and the other as the input feature to make a prediction.
...
For the regression task in \ref{ratingpred}, we use mean squared error (MSE) as the evaluation metric.
...
\begin{tabular}{@{}llcll@{}}
\toprule
                     & MSE   & Improvement over Matrix Factorization &  &  \\ \midrule
Matrix Factorization & 1.561 & -                                     &  &  \\
Feat2Vec             & \textbf{0.480} &  \textbf{69.2} \%                                &  &  \\
DeepCoNN             & 1.441 & 19.6 \%                                &  &  \\ \bottomrule
\end{tabular}
\end{table}
...
In the self-supervised setting, $\Feat2Vec$'s embeddings are able to capture relationships across features that can be twice as better as $\Word2Vec$'s CBOW algorithm on some evaluation metrics.
