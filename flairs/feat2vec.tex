...
Given a dataset with $k$ feature groups $x_1,x_2\dots x_k$, we choose a single categorical feature as the `target'. 
Given a held-out instance, we compute the average of the embeddings vectors $\vec{\phi}$ of all the features except the target $\sum^{k-1}_{i=0}\vec{\phi}_i$ and rank all possible values of the target feature by cosine similarity. 
The rankings are evaluated with according to  their  mean percentile rank (MPR).
$MPR = \frac{1}{N}\sum_{i=1}^N R_i/(\max R)$, where $R_i$ is the rank of the entity under our evaluation procedure for observation $i$.
...
We will use this method to compare the performance of $\Feat2Vec$ algorithm and compare its performance to $\Word2Vec$'s CBOW algorithm for learning embeddings.
