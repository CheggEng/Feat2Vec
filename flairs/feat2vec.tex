...
\author{
  Luis Armona\thanks{Email: \texttt{larmona@stanford.edu} . Affiliation: Department of Economics, Stanford University }, Jos\'e Gonz\'alez Brenes\thanks{Email: \texttt{jgonzalez@chegg.com}}, and Ralph Edezath\thanks{Email: \texttt{redezath@chegg.com}} \\
...
\end{equation}
Other functional forms of $\phi_i$ we have explored in our experiments include:
\begin{itemize}
...
%For example, Principal Component Analysis is a common method to learn embeddings of individual dimensions of  data.
...
%For example, consider a dataset that has a textual description of items--- we may be interested in having an embedding representation for the entire description instead of the individual words.
...
In such a scenario, it is overwhelmingly costly to feed the model all negative labels.
...
Instead, by grouping features using the parameter $\vect{\pmb{\upkappa}}$ in Equation \ref{eq:deepfm}, the model can reason on more abstract entities in the data. By entity, we mean a particular feature group value.
...
With $n$ feature groups, the loss function of self-supervised $\Feat2Vec$ can be shown to be a weighted average of the losses from $n$ distinct supervised $\Feat2Vec$ models\footnote{Proof can be found in the supplemental material at \url{http://web.stanford.edu/~larmona/supplemental_theorem.pdf}}, one for each feature group.
In other words, it optimizes $n$ multi-label classifiers, where each classifier is optimized for a different feature group. Intuitively, in Algorithm \ref{algo:samp}, we sample a target feature group according to $P_{\q_1}$, and then add that group's supervised loss to the total self-supervised loss.


\section{Empirical Results}
\label{sec:evaluation}
For all our experiments, we define a development set and a single test set which is 10\% of the dataset, and a part of the development set is used for early stopping or validating hyper-parameters. In preliminary experiments we noticed that regularization slows down convergence with no gains in prediction accuracy, so we avoid overfitting only by using early stopping.
...
Since our algorithm is designed for multi-modal data, we generalize this task for feature types beyond text as follows.
...
Given a held-out instance, we rank all possible values of the target group by cosine similarity to the source group's embedding $\phi_j( \vect{x}_{\vect{\upkappa}_j})$ for that instance. We then retrieve the ranking of the actual target entity in the test dataset.
Rankings are evaluated with according to  their  mean percentile rank (MPR):
$MPR = \frac{1}{N}\sum_{i=1}^N R_i/(\max_i R_i)$, where $R_i$ is the rank of the entity under our evaluation procedure for observation $i$.
...
%
Wherever possible, we use common hyperparameters across models from \cite{mikolov2013distributed}.
Explicitly, for our unsupervised $\Feat2Vec$ model, we set $\alpha_1=.25$, $\alpha_2=0.75$, and $k=5$.
Our novel feature group weighting parameter, $\alpha_1$, was not tuned, but instead set ex-ante to a value that would highlight $\Feat2Vec$'s ability to prioritize learning of low-complexity feature groups which nonetheless appear in each observation.
...
We now focus on how well $\Feat2Vec$ performs on a real-valued feature.  We expect this task to highlight $\Feat2Vec$'s advantage over token-based embedding learning algorithms, such as $\Word2Vec$. Our rating embedding extraction function (a 3-layer DNN) will require embeddings of numerically similar ratings to be close, while $\Word2Vec$ will treat two numerically different ratings as completely different entities.
...
\footnote{The $\Feat2Vec$ model code is available \href{https://www.dropbox.com/sh/wdr2sgt0z9gj6kb/AABlzw7QhteTYViSoMk3CDZpa?dl=0}{here}. The self-supervised experiments using non-proprietary data are available \href{https://www.dropbox.com/sh/uc07ng403i4ss9a/AAAPshFzdOug_ooeLn4SGR3Ua?dl=0}{here} and the supervised experiment is available \href{https://goo.gl/zEQBiA}{here}.
}.
