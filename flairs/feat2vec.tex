...
We will use this method to compare the performance of $\Feat2Vec$ algorithm and compare its performance to $\Word2Vec$'s CBOW algorithm for learning embeddings. For $\Word2Vec$, each record is treated as a separate document. To incorporate non-text data in $\Word2Vec$, we tokenize all feature values and treat them as words in the $\Word2Vec$ training corpus (e.g. a movie record flagged as belonging to the horror genre has the token  ``genre\_horror'' in its $\Word2Vec$ training document).
%
In our experiments, we set $\alpha_1=.25$, $\alpha_2=0.75$, and $k=5$. Wherever possible, we use the same hyperparameters across embedding models ($\Word2Vec$, $\Doc2Vec$, $\Feat2Vec$).
We chose an $\alpha_1$ value that would highlight $\Feat2Vec$'s ability to prioritize learning of low-complexity feature groups which nonetheless appear in each observation.
...
\footnote{The $\Feat2Vec$ model code is available \href{https://www.dropbox.com/sh/wdr2sgt0z9gj6kb/AABlzw7QhteTYViSoMk3CDZpa?dl=0}{here}. The experiments using non-proprietary data are available \href{https://www.dropbox.com/sh/uc07ng403i4ss9a/AAAPshFzdOug_ooeLn4SGR3Ua?dl=0}{here}.
}.
