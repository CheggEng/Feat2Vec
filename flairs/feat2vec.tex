...
In the self-supervised case, our experiments suggest that $\Feat2Vec$ better expresses the information content in datasets than existing algorithms that do not leverage the structure of the data.
...
Embeddings  have multiple advantages, as they enable more efficient training~\cite{mikolov2013distributed}, and are often used for general-purpose reasoning..

We consider two kind of algorithms that use embeddings:
\textbf{supervised methods}, like matrix factorization, produce embeddings that are highly tuned to a prediction task.
...
For example, an embedding of a movie learned to predict ratings is unlikely to be extremely helpful for an unrelated prediction task.
...
On the other hand, \textbf{self-supervised methods} (sometimes less precisely referred as unsupervised methods) like the $\Word2Vec$ models~\cite{mikolov2013distributed}, are designed to provide embeddings that are useful for a wide-array of downstream predictions tasks.
For example,
...
%however,  in practice,
the embeddings  may be used in analogy solving~\cite{mikolov2013distributed}, or sentiment analysis~\cite{le2014distributed}.
...
In this paper we propose $\Feat2Vec$ as a novel method that allows calculating embeddings of arbitrary feature types, such as text, images, categorical, or categorical data, in a common vector space, for both supervised and self-supervised scenarios.
The contributions of this paper are two-fold.
First, Feat2Vec is the first algorithm that is able to calculate general-purpose embeddings with self-supervision.
Second, we allow calculating embeddings of arbitrary features.
For example, consider the non-trivial work that an engineer needs to do to extend a model like Word2Vec to support  additional features:
the authors of the seminal Doc2Vec~\cite{le2014distributed} paper needed to design a new neural network and a new sampling strategy  to simply add a single feature (Document ID).
...
For example, in supervised tasks, an item with a description (e.g., text features) can be used avoid the cold-start problem by replacing  item embeddings with a description embedding.
%
%
...
% $\Feat2Vec$ extends Factorization Machine in two ways:
% it allows grouping of features and it enables arbitrary feature extraction functions.
We now describe how $\Feat2Vec$ extends the Factorization Machine model by allowing grouping of features, and enabling arbitrary feature extraction functions .
We also report a supervised method to learning Feat2Vec,
as well as a novel self-supervised training procedure.
...
The embeddings of a feature group are then learned via a \textit{feature extraction function}, $\phi_i$, defined for each feature group.
$\Feat2Vec$ will then extract features from each feature group, and build $r$ latent factors from them. In Factorization Machine, all the feature embeddings interact with each other, while in $\Feat2Vec$, the interactions only occur between different feature \textit{groups}.
...
\begin{align}
%\hat y(\vect{x}; \vect{b}, \vect{\upphi}, \vect{\upkappa})=
\hat y(\vec{x},\vec{b},\vec{\phi} ) = \omega \biggl(
                      b_0 +
                      \sum_{i=1 \vphantom{G(i)}}^n b_i x_i  \; + \;
                       \sum_{i =1}^{|\vect{\upkappa}|} \sum_{j=i}^{|\vect{\upkappa}|}   \phi_i( \vect{x}_{\vect{\upkappa}_i}) \dotp \phi_j( \vect{x}_{\vect{\upkappa}_j})
  \biggr)
...
We can use $\Feat2Vec$ to overcome the cold-start problem.
This is only possible when there is an alternative description of the item available (for example an image or a passage of text).
We  address this problem by treating the words as indexed features, but placed within a structured feature group $\upkappa_w$. %the group of word features.
A feature extraction function $\phi$ acts on the features in $\upkappa_w$, and the other features interact with the words only via the output of $\phi$.
