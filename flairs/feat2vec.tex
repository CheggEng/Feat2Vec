...
/Author (Luis Armona, Jose Gonzalez Brenes, Ralph Edezath)
}

\title{Beyond Word Embeddings: Dense Representations for Multi-modal Data}

\author{
  Luis Armona,\textsuperscript{\rm 1} Jos\'e Gonz\'alez Brenes,\textsuperscript{\rm 2} Ralph Edezath\textsuperscript{\rm 2} \\
  \textsuperscript{\rm 1}Department of Economics, Stanford University, \\
  \textsuperscript{\rm 2}Chegg, Inc.\\
  larmona@stanford.edu, jgonzalez@chegg.com, redezath@chegg.com
}
...
In this paper we propose $\Feat2Vec$ as a novel method that  embeds arbitrary feature types, such as text, images, numerical or categorical data, in a common vector space---for both supervised and self-supervised scenarios.
...
\item A deep, fully connected neural network that projects a scalar, such as average rating, into an $r$-dimensional embedding space. This extraction function, by treating the input as numerical instead of categorical, requires inputs close in value (e.g. 4.01 and 4.02 stars) to have similar embeddings.
\end{itemize}
...
Instead, by grouping features using the parameter $\vect{\pmb{\upkappa}}$ in Equation \ref{eq:deepfm}, the model can reason on more abstract entities in the data, where entity is defined to be a particular feature group value.
...
To make results comparables, the $\Feat2Vec$ experiments use the same feature extraction function used by DeepCoNN.
...
\begin{figure*}[th]
\centering
\includegraphics[width=.8\textwidth]{../paper/output/yelp/text_rating_confusionmat_numbers}
...
We now focus on how well our estimated self-supervised $\Feat2Vec$ model performs when predicting a real-valued feature.  We expect this task to highlight $\Feat2Vec$'s advantage over token-based embedding learning algorithms, such as $\Word2Vec$. Our rating embedding extraction function (a 3-layer DNN) will require embeddings of numerically similar ratings to be close, while $\Word2Vec$ will treat two numerically different ratings as completely different entities.
