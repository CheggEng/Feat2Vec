...
$\Feat2Vec$ predicts  a target output  $\tilde y$ from each observation ${\vect{\mathbf x}}$, which can be interpreted as a list of feature groups:
\begin{align}
 ...
 \end{align}
Each of the $n$ feature groups is defined a priori by $\vect \upkappa_i$, which indexes the dimensions in ${\vect{\mathbf x}}$  that belong to the $i$-th group.
Thus, each observation is constructed from a partition of raw features ${\vect{\mathbf x}}$
(e.g. a raw feature can be a pixel, and a group can be a collection of pixels making an image).
The number of dimensions of each group may vary, but all of the feature groups are embedded to the same space via their \textit{feature extraction function} $\phi_i$. The parameters associated with the extraction function $\phi_i$ are denoted $\vect{{\uptheta}}_i$.
...
These extraction functions learn how to transform each input (feature group) into an $r$-dimensional embedding in $\mathbb{R}^r$.
...
\phi_{i}\bigl( \vect{x\vphantom{\theta}}_i; \vect{\uptheta}_{i,r}\bigr )_r= \sum_{a=1}^{d_i}  \theta_{i,r,a} x_{i,a}
\end{equation}
...
= \frac{ \operatorname{count}(\vect{g} )^{\alpha_2}}{\sum\limits_{\vect{j}' \in\mathrm{X_{\upkappa_i}} } \operatorname{count}(\vect{j}'')^{\alpha_2}}, \quad \alpha_2 \in [0,1]
\end{equation}
...
Here, $\tilde y (.)$ denotes the model in Equation $\ref{eq:deepfm}$ with $\omega$ set to the identity function. This can be interpeted as the ``score'' of an observation with respect to $\Feat2Vec$.
 ...
 it accounts for the possibility of sampling an observation with a negative label from Algorithm \ref{algo:samp} that appears identical to one in the training data (with a positive label):
\begin{align}
...
\begin{equation}
\arg\min_{ \vect{\bm\uptheta}} \sum_{\vect{x}^{\,+} \in \mathbf{X}}  \mathcal{L}_\text{self} \bigl( {\vect{x}^{\,+}};  \vect{\bm\uptheta}  \bigr)
\end{equation}
...
-\sum_{\vect{x}^{\,+} \in \mathbf{X}} \biggl( \log\bigl(\hat s(\vect{x}^{\,+}; \vect{\bm\uptheta} )   \bigr) +  \textstyle\sum\limits_{\vect{x}^{\,-}  \sim \q(\cdot|\vect{x}^+)}^k \log\bigl(1-\hat s(\vect{x}^{\,-}; \vect{\bm\uptheta}) \bigr) \biggr)
\end{align}
...
\footnote{The $\Feat2Vec$ model code is available  here: \url{https://github.com/CheggEng/Feat2Vec}. The self-supervised experiments using non-proprietary data are available here: \url{http://web.stanford.edu/~larmona/feat2vec/}. The supervised experiments are available here: \url{https://goo.gl/zEQBiA}.
}.
