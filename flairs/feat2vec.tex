...
We study how to estimate  dense representations when multiple feature types exist (e.g., text, continuous, categorical) within a dataset.
...
On the other hand, \textbf{(ii)~self-supervised methods} (sometimes referred to as unsupervised methods) are not tuned for the prediction task that they are ultimately used for.
For example, $\Word2Vec$ ~\cite{mikolov2013distributed} methods are self-supervised and are often used for transfer learning.
...
%however,  in practice,
Self-supervised embeddings  are evaluated in how useful they are for analogy solving, or sentiment analysis~\cite{le2014distributed} eventhough the loss function is not tuned for either of these tasks.
...
We believe that this is the first general-purpose self-supervised algorithm that is able to calculate embeddings from data with multiple feature types.
...
The structure of $\Feat2Vec$ model significantly expands previously published models.
For example, it improves Factorization Machine~\cite{rendle2010factorization}, in that we allow calculating embeddings for feature groups and feature extraction functions.
StarSpace~\cite{wu2017starspace}  introduces feature groups (they call entities), but are constrained  to be  a ``bag of features''. $\Feat2Vec$ allows continuous features.
StarSpace is limited to only two feature types-- where one feature is treated as a label, and the other is a bag of features, and there is no know self-supervised training method.
In contrast, our model does not require the user to assign a feature as a label, and we propose a self-supervised learning algorithm.
...
$\bm\uptheta$ represents the parameters learned in during training (i.e. the parameters associated with the extraction functions $\phi_i$  (for example Equation~\ref{eq:fully_connected}).
