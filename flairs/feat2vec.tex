...
The number of dimensions $d_i$ of each feature group may vary, but all of them are embedded to the same space via their \textit{feature extraction function} $\phi_i$.
...
Thus, the range of $\phi_i : \mathbb{R}^{d_i} \rightarrow \mathbb{R}^{r}$.
...
Here, $\tilde y (.)$ denotes the model in Equation $\ref{eq:deepfm}$ with $\omega$ set to the logistic loss. This can be interpeted as the ``score'' of an observation with respect to $\Feat2Vec$.
...
-\sum_{\vect{x}^{\,+} \in \mathbf{X}} \biggl( \log\bigl(\hat s(\vect{x\vphantom{uptheta}}^{\,+}; \vect{\bm\uptheta} )   \bigr) +  \textstyle\sum\limits_{\vect{x\vphantom{uptheta}}^{\,-}  \sim \q(\cdot|\vect{x}^+)}^k \log\bigl(1-\hat s(\vect{x}^{\,-}; \vect{\bm\uptheta}) \bigr) \biggr)
\end{align}
