...
On the other hand, \textbf{(ii)~self-supervised methods} (sometimes referred to as unsupervised methods) like $\Word2Vec$ ~\cite{mikolov2013distributed}, are often used for transfer learning.
In this context, transfer learning involves using self-supervised embeddings for predicting an unrelated task.
For example,
...
%however,  in practice,
$\Word2Vec$ embeddings  may be used for analogy solving, or sentiment analysis~\cite{le2014distributed}.
...
Though a recently proposed algorithm, StarSpace~\cite{wu2017starspace}, enables embedding feature types beyond words, it is limited to two features, where one feature is treated as a label.
In contrast, our self-supervised method does not require the user to assign a feature as a label.
...
These functions learn how to embed each vector (feature group) and outputs a vector in $\mathbb{R}^r$.
...
The structure of this model extends both Factorization Machine~\cite{rendle2010factorization}, in that we allow grouping of features,
and StarSpace~\cite{wu2017starspace}, in that we do not constrain the feature groups to be  a ``bag of features'' and allow an arbitrary number of features.
...
The simplest   implementation of $\phi_i$ is a linear fully-connected layer, where the output of the $r$-th entry is:
\begin{equation}
\label{eq:fully_connected}
...
Feat2Vec could be used to predict on unseen items, by learning an embedding from an alternative characterization of the item---for example an item's textual description.
...
$\bm\uptheta$ represents the parameters learned in during training (i.e. the parameters associated with the extraction functions $\phi_i$ in Equation \ref{eq:deepfm}).
...
In the multi-class scenario, if the number of labels is very large, it is common practice to use a binary classifier with implicit sampling~\cite{samplingnotes}.
...
The datasets we use for our labeling experiments only contains positive labels, thus for each training example we sample a $k$ negative labels equal for each positive label.
...
Instead, by grouping subfeatures using the parameter $\vect{\pmb{\upkappa}}$ in Equation \ref{eq:deepfm}, the model can reason on more abstract entities in the data. By entity, we mean a particular feature group value.
...
With $n$ feature groups, self-supervised $\Feat2Vec$ can be shown to be equivalent to optimizing a convex combination of the loss functions of  $n$ supervised $\Feat2Vec$ models with implicit sampling (proof omitted).
In other words, it optimizes $n$ multi-label classifiers, where each classifier  is optimized for a different target (i.e. feature group). Intuitively, during sampling in self-supervised Feat2Vec, we choose a target feature group according to $P_{\q_1}$, and then add that group's supervised loss to the total loss.
...
In contrast, for $\Feat2Vec$, we build 3 feature groups: item (restaurant) and user identifiers, and review text.
...
We evaluate the prediction of the real-valued rating of movies (scale is 0 to 10) in the test dataset by choosing the IMDB rating embedding most similar\footnote{As before, the metric is cosine similarity.} to the embedding of the movie's director, and compute the MSE of the predicted rating in the test dataset.
 $\Feat2Vec$ scores an MSE of 6.6, while  $\Word2Vec$  scores 9.3.
...
To the extent of our knowledge, Self-Supervised $\Feat2Vec$  is the first method able to calculate continuous representations of data with arbitrary feature types without explicit labels.
