...
In contrast, $\Feat2Vec$ allows any number of groups to enter the model, and is compatible with multi-modal data, such as continuous features or ordered sequences.
...
Unlike $\Word2Vec$, a popular self-supervised embedding algorithm, we do not constrain features types to be words.
...
\For {$\vect{x}^{\,+} \in S^+ $}
	\State    $\upkappa_i  \sim \q_1(\{\operatorname{params}(\phi_i)\}_{i=1}^{|\vect{\upkappa}|},\alpha_1) $	\Comment Draw a random \begin{flushright} feature group\end{flushright}
	\For {$j \in \{1,\ldots,k\}$}
...
Self-supervised $\Feat2Vec$ has interesting theoretical properties.
...
Given a dataset with $n$ feature groups $g_1,g_2,\ldots g_n$ we choose one group $g_y$ as the ``target'',  and choose another single feature group as the predictor ($g_x$).
During training, self-supervised Feat2Vec does not treat the target group differently than any other group.
Given a held-out instance, we rank all possible values of the target group by cosine similarity to the predictor group's embedding $\phi_x( \vect{x}_{\vect{\upkappa}_x})$ for that instance. We then retrieve the ranking of the actual target entity for the held-out instance in the test dataset.
Rankings are evaluated according to  their  mean percentile rank:
...
For Word2Vec and Doc2Vec, the review text embedding is the average of a review text's word embeddings, analogous to the context vector used for learning in these algorithms. Figure \ref{fig:confmat} reports the confusion matrices for each model from this experiment.
