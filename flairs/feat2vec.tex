...
Given a dataset with $n$ feature groups $g_1,g_2,\ldots g_n$ we have trained a $\Feat2Vec$ model on, we choose a single group $g_i$ as the ``target'' and a group $g_j$ as the predictor.
...
We will use this method to compare the performance of $\Feat2Vec$ algorithm and compare its performance to $\Word2Vec$'s CBOW algorithm for learning embeddings. In our experiments, we set $\alpha_1=.25$, $\alpha_2=0.75$, and $k=5$. We evaluate self-supervised $\Feat2Vec$  on 3 datasets:
