...
#create sample DF
#that we will implicitly sample from
...
#print "----"
#sample_strat = SamplingStrategy(features=features,items =[items3,items12] ,alpha=.5,bias=10)
#P = sample_strat.get_probability(testdata)

#ImpS = ImplicitSampler(testdata,features=features,model_features=model_features,batch_size=batch_size,
#                       sampling_strategy=sample_strat,
#                       sampling_features=sampling_features,sampling_items=itemset_list)

#print sample_strat.get_probability(testdata)[1]
print "^*^*^*^*^*"
ind=1
for x,y,w in ImpS.keras_generator():
